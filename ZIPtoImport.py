...
if __name__ == '__main__':
    ...
    session.execute(
        """
        DROP TABLE IF EXISTS million_song.song_table_full_new
        """
    )

    session.execute(
        """
        CREATE TABLE IF NOT EXISTS million_song.song_table_full_new (
            SongNumber bigint,
            SongID text,
            albumName text,
            albumID bigint,
    ...
    try:
        os.mkdir(csv_dump_path)
    except OSError:
        print "Path already exists; will reuse."

    for root, dirs, files in os.walk(base_dir):
        for fs in files:
            if fs.endswith(archive_ext):
                path = os.path.join(root, fs)
                print(path)

                with ZipFile(path) as zipArchive:
